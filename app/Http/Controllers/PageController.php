...
class PageController extends Controller
{
    ...
    {
           /*  $counter=Operations::select(DB::raw('count(operations.id) as nombre','marchand et ville name'))
            ->join('terminals', 'operations.terminal_id', '=', 'terminals.id')
            ->rightJoin('marchants', 'marchants.id', '=', 'terminals.marchants_id')
            ->join('villes', 'villes.id', '=', 'marchants.villes_id')
            ->groupBy('marchants.Name','villes.Name')->get();  */

            $dataodFiveOperation =Operations::join('users','operations.user_id','=','users.id')
            ->join('terminals','operations.terminal_id','=','terminals.id')
            ->join('marchants','marchants.id','=','terminals.marchants_id')
            ->join('villes','villes.id','=','terminals.villes_id')
            ->groupBy('marchants.Name' , 'operations.created_at')
            ->orderByDesc('Nombre')
            ->limit(5)
            ->select(DB::raw('count(operations.id) as Nombre,operations.created_at as date,marchants.Name'))->get();

            $data = [Operations::whereDay('created_at',Date('d'))->count(),  Operations::whereMonth('created_at',Date('m'))->count(),Terminal::count(), Marchant::count()];

           
/*                 $data = Operations::join('terminals','operations.terminal_id','=','terminals.id')
                 ->rightJoin('marchants','terminals.marchants_id','=','marchants.id')
                 ->rightJoin('villes','marchants.villes_id','=','villes.id')
                 ->groupBy('villes.Name')
         ...
         //return $dataodFiveOperation;
      return view('operation.dashboardOperations', ['layout' => 'side-menu', 'data'=> $data , 'operation'=> $dataodFiveOperation
