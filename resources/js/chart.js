...
(function (cash) {
    ...
    if (cash("#report-pie-chart").length) {
        ...
        axios.get('https://operations.easypay.cm/api/operationsjs')
        .then(response => response)
        .then(operations => {
           // console.log('Success:', operations.data);
           let clrs =  getRandomColor(operations.data[1].length)
            for(var i=0; i <operations.data[1].length; i++){
                cash("#legendChartPie").append('<div class="flex items-center mt-4"><div class="w-2 h-2 rounded-full mr-3" style="background-color: '+clrs[i] +';"></div><span class="truncate">'+operations.data[0][i]+'</span> <div class="h-px flex-1 border border-r border-dashed border-gray-300 mx-3 xl:hidden"></div><span class="font-medium xl:ml-auto">'+operations.data[1][i]+'</span></div>');
               // console.log( cash("#legendChartPie").html());
            }
            var data = {
            labels: operations[0],
            series: operations[1]
            ...
            let myPieChart = new chart(ctx, {
                type: "pie",
                data: {
                    labels: operations.data[0],
                    datasets: [
                        {
                            data: operations.data[1],
                            backgroundColor: clrs,
                            hoverBackgroundColor: clrs,
                            borderWidth: 5,
    ...
    function getRandomColor(lgth){
        var letr = "0123456789ABCDEF";
        let arr = new Array();
        var clr = "#";
        for(var j=0; j<lgth ; j++){
            /*for(var i=0; i<6 ; i++){
                //clr += letr[Math.floor(Math.random()*16)];

            }*/
            arr[j]='#'+(Math.random()*0xFFFFFF<<0).toString(16);
        }
